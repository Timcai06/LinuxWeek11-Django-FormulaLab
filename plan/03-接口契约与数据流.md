...
其中 `PaperCodeEditor` 使用 CodeMirror 6 承载论文源码编辑，`PaperFileDialog` 负责新建/重命名文件的路径输入，`PaperPreview` 暂时展示源码预览，后续再替换为 PDF 或结构化 LaTeX preview。Vite 将 CodeMirror 依赖拆分为独立 `codemirror` chunk，降低主编辑器入口体积。
