...
assert.match(introSource, /addLabel\("calibrationHold"/, "HeroIntroDirector should include a black-field calibration hold before the warm surface arrives.");
...
assert.match(introSource, /DrawSVGPlugin/, "HeroIntroDirector should use DrawSVG for the blueprint vector construction layer.");
assert.match(introSource, /brand-intro-vector-system/, "HeroIntroDirector should include a dedicated SVG vector construction layer.");
assert.match(introSource, /brand-intro-vector-path/, "HeroIntroDirector should draw measured vector paths during the opening.");
assert.match(introSource, /brand-intro-vector-node/, "HeroIntroDirector should animate vector nodes as construction anchors.");
assert.match(introSource, /brand-intro-vector-label/, "HeroIntroDirector should include restrained vector labels for the construction pass.");
...
assert.match(introSource, /calibrationFontSize[\s\S]*calibrationFormulaLeft[\s\S]*calibrationLabLeft[\s\S]*calibrationTop/, "HeroIntroDirector should compute a centered calibration title state from real DOM measurements.");
assert.match(introSource, /fontSize:\s*calibrationFontSize[\s\S]*letterSpacing:\s*"0\.02em"/, "HeroIntroDirector should use real font-size layout for the small centered calibration title instead of transform scaling.");
...
assert.match(introCssSource, /\.brand-intro-vector-system/, "Brand intro CSS should own the SVG vector construction layer.");
assert.match(introCssSource, /\.brand-intro-vector-path/, "Brand intro CSS should style blueprint vector paths.");
assert.match(introCssSource, /\.brand-intro-vector-node/, "Brand intro CSS should style construction anchor nodes.");
assert.match(introCssSource, /\.brand-intro-vector-label/, "Brand intro CSS should style restrained vector labels.");
assert.match(introCssSource, /\.brand-intro-curtain[\s\S]*background:[\s\S]*linear-gradient\(108deg, rgba\(0, 0, 0, 1\)/, "Brand intro should begin from a true black-field curtain.");
