...
assert.match(
  curtainSource,
  /getLandingMotionRuntime[\s\S]*createScrollFrameGate[\s\S]*runtime\.subscribe\(\(frame\) => \{[\s\S]*frameGate\.shouldUpdate\(frame\)[\s\S]*frame\.transitions\.greenLiquidProgress[\s\S]*frame\.transitions\.blackLiquidProgress/,
