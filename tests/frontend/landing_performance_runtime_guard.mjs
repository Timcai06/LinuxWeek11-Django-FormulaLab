...
assert.match(
  curtainSource,
  /getLandingMotionRuntime[\s\S]*runtime\.subscribeRenderer\(\{[\s\S]*id:\s*"morph-curtain"[\s\S]*includeTransitionSettling:\s*true[\s\S]*frame\.transitions\.greenLiquidProgress[\s\S]*frame\.transitions\.blackLiquidProgress/,
